...
import { FiHome, FiBook, FiUsers, FiBell, FiUser, FiLogOut, FiMenu, FiX, FiMoon, FiSun } from 'react-icons/fi';
import { logoutUser } from '../store/authSlice';
import { useTheme } from '../context/ThemeContext';
import RhythmDanceLogo from '../assets/rhythmdance.png';

const navItems = [
  { to: '/faculty/dashboard', icon: FiHome, label: 'Dashboard' },
  { to: '/faculty/classes', icon: FiBook, label: 'My Classes' },
  { to: '/faculty/students', icon: FiUsers, label: 'My Students' },
  { to: '/faculty/notifications', icon: FiBell, label: 'Notifications' },
  { to: '/faculty/profile', icon: FiUser, label: 'My Profile' },
];
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex">
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 z-30 flex flex-col transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-display font-bold text-gray-900 dark:text-white truncate">Faculty</span>
          </div>
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-accent-100 dark:bg-accent-900/30 flex items-center justify-center">
              <span className="text-accent-600 font-semibold">{user?.name?.[0]}</span>
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{user?.name}</p>
              <p className="text-xs text-accent-600 dark:text-accent-400">Faculty</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col">
        <header className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold text-gray-900 dark:text-white">Faculty Portal</h1>
